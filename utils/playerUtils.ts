export const calculatePlayerSummary = (
  playerWarRecords: any[],
  leagueWarRecords: any[],
  ...
  const calculateSummary = (records: any[]) => {
    ...
    records.forEach((record) => {
      const playerData =
        record.content.clan.members.find(
          (member: any) => member.name === selectedPlayer
        ) ||
        record.content.opponent.members.find(
          (member: any) => member.name === selectedPlayer
        );

      if (playerData) {
        totalWars++;
        if (playerData.attacks?.length) {
          totalStars +=
            playerData.attacks?.reduce(
              (sum: number, attack: any) => sum + attack.stars,
              0
            ) || 0;
          totalDestruction +=
            playerData.attacks?.reduce(
              (sum: number, attack: any) => sum + attack.destructionPercentage,
              0
            ) || 0;
          totalAttacks += playerData.attacks?.length || 0;
          const attacksPerMember = Number(record.content.attacksPerMember) || 0;
          missedAttacks += Math.max(
            0,
            attacksPerMember - (playerData.attacks?.length || 0)
          );
        }
      }
    });
    ...
    const averageDestruction =
      totalAttacks > 0 ? (totalDestruction / totalAttacks).toFixed(2) : "0";
  ...
  return {
    ...
    combinedSummary: {
      ...
      averageStars: (
        (parseFloat(normalWarSummary.averageStars) *
          normalWarSummary.totalWars +
          parseFloat(leagueWarSummary.averageStars) *
            leagueWarSummary.totalWars) /
        (normalWarSummary.totalWars + leagueWarSummary.totalWars || 1)
      ).toFixed(2),
      averageDestruction: (
        (parseFloat(normalWarSummary.averageDestruction) *
          normalWarSummary.totalWars +
          parseFloat(leagueWarSummary.averageDestruction) *
            leagueWarSummary.totalWars) /
        (normalWarSummary.totalWars + leagueWarSummary.totalWars || 1)
      ).toFixed(2),
      missedAttacks:
        normalWarSummary.missedAttacks + leagueWarSummary.missedAttacks,
    },
  };
};
...
export const calculateAttackPerformance = (
  savedAttacks: any[],
  selectedPlayer: string
) => {
  const playerAttacks = savedAttacks.filter(
    (attack) => attack.member === selectedPlayer
  );

  if (!playerAttacks.length) return [];

  const attackPerformance: Record<
    string,
    {
      count: number;
      totalStars: number;
      totalDestruction: number;
      higherThCount: number;
      equalThCount: number;
      lowerThCount: number;
    }
  > = {};
